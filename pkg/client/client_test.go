...
var _ = Describe("Reporting client", func() {
	...
	It("can report usage", func() {
		usageClient, err := newUsageClient(buildEmptyPayloadGetter(), instanceMetadata, reportingServiceClient)
		...
		cancelFunc := usageClient.StartReportingUsage(context.TODO(), time.Millisecond * 500)
